...
public class LoadDivDaoImpl implements LoadDivDao {
	...
	public int checkTmpltHeader(MultipartFile file) {
	    try {
			...
			boolean currHoldHeader = fmt.formatCellValue(row.getCell(index++)).toString().contains(CURR_HOLD);
			boolean purchasedDate = fmt.formatCellValue(row.getCell(index++)).toString().contains(PURCHASED_DATE);
			boolean soldDate = fmt.formatCellValue(row.getCell(index++)).toString().contains(SOLD_DATE);
			boolean createdBy = fmt.formatCellValue(row.getCell(index++)).toString().contains(CREATED_BY);
